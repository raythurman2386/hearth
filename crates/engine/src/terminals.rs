...
//! hearth's `terminals.ts` over `portable-pty`).
//!
...
//!   owner, so the per-user owner re-checks from hearth's Router land with real
...
use hearth_doc::TERMINAL_OUTPUT_BATCH_MS;
