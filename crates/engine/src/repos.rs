...
//! browser (feature-inventory §3.5; port of hearth's `repos.ts` + `folder-lister.ts`).
//!
...
//! created under `~/.hearth/worktrees/<repoName>/<worktreeName>` (NOT the data
//! dir — worktrees are user-facing working checkouts), with an auto-generated name +
//! matching `hearth/<name>` branch. `HEARTH_WORKTREES_DIR` overrides the root.
//!
...
const NOUNS: &[&str] = &[
    "otter", "harbor", "falcon", "cedar", "meadow", "hearth", "delta", "ember", "lynx", "maple",
...
/// worktrees are user-facing working checkouts. `HEARTH_WORKTREES_DIR` overrides
/// (test isolation); empty reads as unset.
fn default_worktrees_root() -> PathBuf {
    std::env::var_os("HEARTH_WORKTREES_DIR")
        .filter(|s| !s.is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| home_dir().join(".hearth").join("worktrees"))
}
...
impl Repos {
    ...
    /// comes from `$HEARTH_WORKTREES_DIR` or `~/.hearth/worktrees`.
    ...
    /// remotes, and tags seed the walk, so Hearth's internal refs never leak
    ...
    /// `{worktrees_root}/<repoName>/<generatedName>`, on a fresh `hearth/<name>`
    /// branch off `branch`.
    pub async fn create_worktree(
        ...
        for attempt in 0..50u64 {
            ...
            if !base.join(&candidate).exists() && !existing.contains(&format!("hearth/{candidate}"))
            {
                name = Some(candidate);
                break;
            }
        ...
        let branch_name = format!("hearth/{name}");
        self.git(
            &[
                "worktree",
                "add",
    ...
    /// Rename a hearth-created worktree branch after its chat's generated title
    /// (port of hearth's `renameWorktreeBranch`). Guards:
    /// - respect an external checkout/rename: only act while the worktree is still
    ///   on `expected_branch` AND that branch is the original `hearth/<folderName>`;
    ...
    pub async fn rename_worktree_branch(
        ...
        let folder = worktree_path
            ...
            .map(|n| n.to_string_lossy().to_string())
            .unwrap_or_default();
        if current != expected_branch || expected_branch != format!("hearth/{folder}") {
            return Ok(current);
        }
    ...
    /// Deletes the worktree's branch ONLY when hearth created it (`hearth/…`) — the
    ...
    /// timeout the thread is simply abandoned (the hearth backend's disposable
...
fn list_folders_blocking(target: &Path) -> Result<FolderListing, EngineError> {
    let read = std::fs::read_dir(target).map_err(|e| match e.kind() {
        std::io::ErrorKind::PermissionDenied => {
            EngineError::Other("Hearth doesn't have access to this folder on the device.".into())
        }
...
/// Turn a generated chat title into the semantic portion of a Hearth branch
/// (port of hearth's `worktreeBranchFromTitle`). Hearth NFKD-normalizes accented
...
pub fn worktree_branch_from_title(title: &str) -> String {
    ...
    format!("hearth/{}", if slug.is_empty() { "update" } else { slug })
}
