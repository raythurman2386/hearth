...
//!
//! Pragmatic port of hearth's `sessions.ts` (spec: feature-inventory §3.2):
...
//!
//! Scope notes: sessions are keyed by chat id (one live run per chat). Hearth's pulse
...
use hearth_harness::{CancellationToken, Harness, RunControls, SteerMessage};
use hearth_proto::{
...
/// directory — hearth sessions.ts:563 "harness session stores are keyed by
...
struct RuntimeConfig {
    ...
    cwd: String,
    sandbox: hearth_proto::SandboxLevel,
    auto_approve: bool,
    worktree: Option<hearth_proto::WorktreeSpec>,
}
...
struct Inner {
    ...
    /// (hearth kept the same pair on `chats.harness_session_id`). An empty
...
impl SessionsEngine {
    ...
    pub fn any_active(&self) -> bool {
        lock(&self.inner.statuses).values().any(|s| {
            matches!(
                s.status,
                hearth_proto::SessionStatus::Working | hearth_proto::SessionStatus::AwaitingInput
            )
        })
    }
    ...
    ///   (hearth's persistent-session routing); otherwise any live run is interrupted
    ...
    async fn dispatch_inner(
        ...
        // Engine-owned resume (hearth sessions.ts:736 — every dispatch read the
    ...
    pub async fn interrupt(&self, chat_id: &str) -> Result<bool, EngineError> {
        ...
        // Unpark any blocked question FIRST (mirrors hearth: harness teardown can await a
    ...
    /// the remembered harness session (hearth: "not just eulogized";
    ...
    pub fn recover_stale(&self) -> Result<usize, EngineError> {
        ...
        for chat_id in stale {
            ...
            // same harness conversation (hearth recoverDraft, sessions.ts:538).
            ...
            tokio::spawn(async move {
                ...
                let request = sessions
                    ...
                    // Last resort: the journal's own cwd (hearth's draft config)
...
impl Inner {
    ...
    /// engine restart (hearth sessions.ts:1039).
    ...
    /// (hearth sessions.ts:736, looked up on every dispatch):
...
impl SubagentSink {
    ...
    fn finish(mut self, device_id: &str, status: MessageStatus) -> Option<String> {
        ...
        let entries = hearth_doc::join_continuation_entries(self.doc.read_entries().ok()?);
...
async fn drive_run(
    ...
    // PERSISTENT SESSION (hearth runsBySession): a completed turn on a
    ...
    // a session nobody comes back to (hearth SESSION_IDLE_MS).
    ...
    // moment output flows again, and nothing is lost. `HEARTH_TURN_QUIESCE_MS`
    ...
    let quiesce_after: Option<std::time::Duration> = match std::env::var("HEARTH_TURN_QUIESCE_MS")
        .ok()
    ...
    // push the window during real thinking. `HEARTH_SELF_TURN_QUIESCE_MS`
    // overrides; 0 falls back to the normal window. An explicit
    // `HEARTH_TURN_QUIESCE_MS=0` still disables the watchdog entirely.
    let self_quiesce_after: Option<std::time::Duration> =
        match std::env::var("HEARTH_SELF_TURN_QUIESCE_MS")
            .ok()
            .and_then(|v| v.parse::<u64>().ok())
        {
            Some(0) => None,
    ...
    let final_status = loop {
        let event: AgentEvent = tokio::select! {
            ...
            // Idle reaper (hearth SESSION_IDLE_MS): a parked persistent session
            ...
            _ = tokio::time::sleep_until({
                ...
                && !folded.iter().any(|p| match p {
                    MessagePart::Tool { id, resolved: false, .. } => {
                        id != hearth_proto::LIVE_PLAN_TOOL_ID
                    }
        ...
        } = &event
        {
            ...
            if chip_streaming {
                ...
                hearth_doc::fold_event_into_parts(&mut folded, &event);
                if !dirty {
                    dirty = true;
            ...
            if let Some(sink) = subagents.get_mut(parent_tool_use_id) {
                ...
                hearth_doc::fold_event_into_parts(&mut sink.folded, sub_event);
                sink.dirty = true;
        ...
        match &event {
            ...
            AgentEvent::ToolCall { id, .. } if id == hearth_proto::LIVE_PLAN_TOOL_ID => {}
            AgentEvent::ToolResult { id, .. } if id == hearth_proto::LIVE_PLAN_TOOL_ID => {}
            AgentEvent::ToolCall { id, .. } => {
        ...
        // Defensive rule from hearth: a mid-run SessionStarted re-emission (Claude SDK
        ...
        if !skip_fold {
            ...
            // journal. To reintroduce: `hearth_doc::sidecar_payload(&event)`
...
mod tests {
    ...
    use hearth_proto::{HarnessId, RunRequest, SandboxLevel};
    ...
    fn request() -> RunRequest {
        RunRequest {
            ...
            reasoning: Some(hearth_proto::ReasoningLevel::High),
