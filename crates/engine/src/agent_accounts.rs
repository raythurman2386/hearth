...
//! (feature-inventory §3.7 "Agent accounts"; port of hearth's `agent-accounts.ts`).
//!
...
//!   whole-account session tokens, which hearth never reads.
//!
...
//! (`/usage` in Claude Code, `/status` in Codex). Unlike hearth (fetch on every
...
impl AgentAccounts {
    ...
    /// Cursor: the SDK's own PKCE browser flow, driven through the hearth shim
    ...
    async fn start_cursor_login(&self) -> Result<AgentLoginStart, EngineError> {
        ...
        let home = self
            ...
            .join(format!(".login-{login_id}"));
        std::fs::create_dir_all(&home)?;
        let mut cmd = hearth_harness::cursor::login_command(&home.join("auth.json"))
            .await
            .map_err(|e| {
                let _ = std::fs::remove_dir_all(&home);
