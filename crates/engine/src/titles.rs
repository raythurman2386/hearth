...
//! untitled chat, name it with the harness's cheapest model (port of hearth's
...
//!    last listed model — hearth's `cheapestModel`);
...
//!    retrying on hearth's short backoff ladder; fall back to the prompt's first
//!    words when every attempt produces nothing;
//! 4. re-check the title (a user rename during generation wins);
//! 5. when the chat sits in a hearth worktree (`hearth/<name>` branch), rename the
...
use hearth_harness::{CancellationToken, RunControls, SteerMessage};
use hearth_proto::{
...
/// couple of times with a short backoff before falling back (hearth's ladder).
...
impl TitleGenerator {
    ...
    async fn generate(
        ...
        // hearth/<name> branch (guards live inside rename_worktree_branch).
        if let (Some(chat_cwd), Some(branch)) = (&latest.cwd, &latest.branch)
            && branch.starts_with("hearth/")
        {
            match self
                .inner
                .repos
...
/// The cheapest model a harness offers (hearth's `cheapestModel` heuristic):
...
async fn collect_text(
    harness: &dyn hearth_harness::Harness,
    request: RunRequest,
