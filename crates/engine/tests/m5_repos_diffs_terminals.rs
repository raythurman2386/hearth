...
use hearth_proto::{GitHistoryRefKind, TerminalEvent};
...
async fn repos_round_trip_add_branches_worktrees() {
    ...
    // Worktree add: hearth/<name> branch, isolated dir under the test root.
    let worktree = repos
        .create_worktree(&repo_dir, "main")
        .await
        .expect("worktree");
    assert!(
        worktree.branch.starts_with("hearth/"),
        "branch: {}",
        worktree.branch
    );
    ...
    // worktree's hearth/<name> branch maps to its linked-checkout path, and
    ...
    // Delete: dir removed, hearth branch removed, refs pruned.
    repos
    ...
    assert!(
        ...
        "hearth branch deleted: {branches:?}"
    );
...
async fn diff_file_text_returns_both_checked_sources() {
    ...
    let escape = hearth_proto::DiffFileSummary {
...
async fn diff_capture_truncates_at_patch_cap() {
    ...
    assert!(snapshot.patch.contains("# Hearth diff truncated"));
}
...
async fn rpc_dispatch_for_m5_methods() {
    ...
    unsafe { std::env::set_var("HEARTH_WORKTREES_DIR", tmp.path().join("worktrees")) };
    let core = assemble(&tmp.path().join("data"));
    let client = hearth_rpc::memory_client(core.rpc_service());
