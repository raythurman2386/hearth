...
//!    existed in the agent's session data, was absent from hearth's doc).
...
use hearth_doc::{MessagePart, MessageRole, MessageStatus, SessionMessageEntry};
use hearth_engine::{EngineCore, HarnessRegistry, SteerOutcome};
...
fn init_quiesce_env() {
    ...
    ONCE.call_once(|| {
        ...
        unsafe { std::env::set_var("HEARTH_TURN_QUIESCE_MS", QUIESCE_MS.to_string()) };
    });
}
