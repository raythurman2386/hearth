...
//! variant against a real edge runs behind `#[ignore]` (HEARTH_EDGE_WS, like
//! hearth-sync's edge_convergence test).
...
async fn bridge(
    a: &EngineCore,
    b: &EngineCore,
) -> hearth_sync::registry::mock_server::MockRegistryServer {
    let server = hearth_sync::registry::mock_server::MockRegistryServer::start().await;
...
async fn two_engines_share_a_workspace() {
    ...
    let client_b = hearth_rpc::memory_client(b.rpc_service());
    client_a
        .call(
            methods::MUTATE,
            serde_json::json!({
    ...
    wait_for(
        || {
            handle.doc().read_entries().unwrap_or_default().iter().any(|e| {
                e.parts.iter().any(
                    |p| matches!(p, hearth_doc::MessagePart::Text { text, .. } if text == "From cursor"),
                )
            })
        },
...
///
/// ```sh
/// HEARTH_EDGE_WS=ws://127.0.0.1:8787 cargo test -p hearth-engine -- --ignored
/// ```
#[tokio::test]
#[ignore = "requires a live edge: set HEARTH_EDGE_WS (e.g. ws://127.0.0.1:8787)"]
async fn two_engines_converge_through_a_real_workspace_room() {
    use hearth_engine::doc_host::EdgeConfig;

    let base = std::env::var("HEARTH_EDGE_WS")
        .expect("set HEARTH_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:8787");
...
async fn legacy_workspace_doc_migrates_instantly_on_first_boot() {
    use hearth_proto::{Chat, Device, Session, Space};
    ...
    {
        let store = hearth_sync::DocsStore::open(&org_dir).expect("open store");
        let legacy = hearth_doc::WorkspaceDoc::new();
    ...
    let store = hearth_sync::DocsStore::open(&org_dir).expect("reopen store");
    assert!(
        store
            .load_snapshot(hearth_doc::REGISTRY_DOC_ID)
