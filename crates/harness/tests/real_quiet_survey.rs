...
//!
//!   SURVEY_RUNS=3 cargo test -p hearth-harness --test real_quiet_survey -- --ignored --nocapture
//!
...
async fn real_all_harnesses_quiet_survey() {
    ...
    let agents: Vec<(&str, AgentCtor)> =
        vec![("grok", AcpHarness::grok), ("raven", AcpHarness::raven)];
