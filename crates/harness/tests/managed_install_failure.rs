...
//!
//! Single-test binary: it mutates PATH/SHELL/HEARTH_* env process-wide.

#![cfg(unix)]

use std::os::unix::fs::PermissionsExt;

use hearth_harness::{AcpHarness, Harness, HarnessError, RunControls};
use hearth_proto::{RunRequest, SandboxLevel};
use tokio::sync::mpsc;
...
async fn silent_npm_enoent_death_surfaces_decoded_error() {
    ...
    unsafe {
        std::env::set_var("HEARTH_ADAPTERS_DIR", dir.path().join("adapters"));
        std::env::set_var("HEARTH_NO_LOGIN_SHELL", "1");
