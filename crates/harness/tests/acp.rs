...
use hearth_harness::{AcpHarness, CancellationToken, Harness, RunControls, SteerMessage};
use hearth_proto::{
...
async fn happy_path_maps_chunks_tools_diffs_plans_and_commands() {
    ...
    assert!(events.contains(&AgentEvent::ToolCall {
        id: "t1".into(),
        call: ToolCall::Exec {
            command: "cargo test -p hearth-harness".into()
        },
    }));
    ...
    assert!(exec_output.starts_with("   Compiling hearth-harness"));
...
async fn config_options_apply_requested_model_and_effort() {
    ...
    req.reasoning = Some(hearth_proto::ReasoningLevel::Medium);
...
async fn grok_subagent_lifecycle_tails_the_disk_transcript_into_tagged_events() {
    ...
    let tool = pos(&|e| {
        matches!(
            e,
            AgentEvent::ToolCall { id, call: hearth_proto::ToolCall::Exec { command } }
