...
//!
//! ```sh
//! HEARTH_EDGE_WS=ws://127.0.0.1:27640 cargo test -p hearth-sync --test registry_edge -- --ignored
//! ```
...
use hearth_proto::{Chat, Session, SessionStatus};
use hearth_sync::RegistryClient;
...
fn edge_url(org: &str, user: &str) -> String {
    let base = std::env::var("HEARTH_EDGE_WS")
        .expect("set HEARTH_EDGE_WS to the edge origin, e.g. ws://127.0.0.1:27640");
...
#[tokio::test]
#[ignore = "requires a live edge: set HEARTH_EDGE_WS (e.g. ws://127.0.0.1:27640)"]
