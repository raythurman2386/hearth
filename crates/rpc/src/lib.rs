//! hearth-rpc — the typed control plane (UiRpc / ControlRpc) over WebSocket + in-memory
...
//! byte transports), matching the shape of hearth's Effect RPC without the Effect runtime:
//!
...
pub use device_room::{
    ...
    NudgeHandler, PeerLiveness, PeerLivenessProbe, PeerUrlFactory, StaticToken, TokenSource,
...
pub mod methods {
    ...
    /// Live sync introspection (`hearth sync` / debug surfaces): per-room
    ...
    /// Pushed edge-connectivity posture (`hearth_proto::Connectivity`):
    ...
    /// In-flight queued-attachment transfers (`hearth_proto::TransferProgress`
