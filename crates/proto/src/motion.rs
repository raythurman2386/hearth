//! Loader motion — the pure math behind hearth's loading indicators.
//!
//! These are the curves and constants the gpui viewport animates with
//! (`hearth-ui/src/motion.rs`, `hearth-ui/src/loaders.rs`), lifted here so any
...
/// Hearth loader pulse period.
pub const HEARTH_PULSE_MS: u64 = 2_400;
/// Gradient matrix spinner wave period.
pub const GRADIENT_SPIN_MS: u64 = 750;

/// Cells in the hearth wave loader.
pub const HEARTH_CELLS: usize = 5;
/// Side length of the gradient spinner matrix.
pub const MATRIX_SIDE: usize = 3;

/// Hearth loader cells rest at this opacity between pulses.
...
/// Per-row tints of the gradient matrix spinner — hearth's "sunrise" gradient
...
/// Hearth loader cell opacity for a phase: 0.08 → 1 → 0.08.
pub fn pulse_opacity(phase: f32) -> f32 {
    PULSE_MIN_OPACITY + (1.0 - PULSE_MIN_OPACITY) * pulse_wave(phase)
}

/// Hearth loader cell scale for a phase: 0.9 → 1 → 0.9.
...
/// ported from hearth's `gradient-spin-pulse` keyframes: full at the cycle
...
/// The hearth mark's pixels — `[x, y]` of each 100×100 cell on the 820×940
/// canvas (hearth's `logo.tsx` CELLS), shared by the static mark and the
/// animated loader.
#[rustfmt::skip]
...
/// Per-cell stagger along the hearth's flight axis. The stagger *adds* phase
