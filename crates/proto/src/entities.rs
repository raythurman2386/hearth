...
//!
//! In hearth these were synced Postgres rows; in hearth they live in the per-org
...
pub struct Device {
    ...
    /// First registration time (hearth devices.created_at — the Devices page
...
pub struct Chat {
    ...
    /// continuity across engine restarts (hearth's `chats.harness_session_id`).
...
pub struct Worktree {
    ...
    pub path: String,
    pub branch: String,
    /// Generated worktree folder name (`hearth/<name>` is its branch).
    #[serde(default)]
    pub name: String,
