//! Animation kit — the hearth motion catalog as reusable helpers over gpui
...
//! - `hearth-pulse` 2.4s staggered cell opacity 0.08→1, scale 0.9→1 (loaders)
...
/// hearth's signature entrance curve — CSS `cubic-bezier(0.16, 1, 0.3, 1)`.
...
/// (`transition-colors` et al. carry it unless overridden; hearth never does).
...
/// Hearth loader pulse period: 2.4s.
pub const HEARTH_PULSE: MotionSpec = MotionSpec::new(2400, EASE);
...
/// (hearth also scales 0.96→1; divs have no scale transform in gpui — approximated.)
...
/// Hearth-pulse floor opacity.
// The loader constants and math live in `hearth_proto::motion` (pure phase
...
// the pointer enters. The original hearth puts Tailwind `transition-colors`
...
/// Dev/measurement knob (`HEARTH_MOTION_SCALE`, default 1): stretches every
/// catalog timeline by this factor — e.g. `HEARTH_MOTION_SCALE=10` slows the
...
pub fn speed_scale() -> f32 {
    ...
    *SCALE.get_or_init(|| {
        std::env::var("HEARTH_MOTION_SCALE")
            .ok()
...
mod tests {
    ...
    #[test]
    fn catalog_timings_match_hearth() {
        ...
        assert_eq!(HEARTH_PULSE.duration_ms, 2400);
