...
//! `remend`, ported to hearth's incremental parser).
//!
...
//! handlers don't apply here — hearth renders raw HTML literally and has no
//! math):
...
pub const PENDING_LINK_URL: &str = "hearth:pending-link";
...
mod tests {
    ...
    fn links_mend_to_pending_sentinel() {
        mends("[docs](https://x.dev/lo", "[docs](hearth:pending-link)");
        mends("[docs](", "[docs](hearth:pending-link)");
        mends("see [do", "see [do](hearth:pending-link)");
        mends("![alt](https://x/i.p", "![alt](hearth:pending-link)");
        stays("see [");
    ...
    fn link_urls_allow_nested_parens() {
        ...
        mends("[a](https://x.dev/(y", "[a](hearth:pending-link)");
    }

    #[test]
    fn emphasis_inside_link_text_closes_inside() {
        mends("[**a", "[**a**](hearth:pending-link)");
        mends("**a [b", "**a [b](hearth:pending-link)**");
    }
