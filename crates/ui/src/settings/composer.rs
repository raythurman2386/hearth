...
//! (hearth parity: localStorage `hearth.composer.defaults:v1`, defaults.ts).
//!
...
/// Remembered model per harness — id plus display label, mirroring hearth's
...
impl ComposerDefaults {
    ...
    /// Remember a pick (hearth `saveDefaults({ harness, modelByHarness })`).
