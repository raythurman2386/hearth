...
use crate::motion::{self, HEARTH_PULSE};
...
/// The floating-menu surface (hearth `.glass-surface` + `menuSurface`):
...
/// One menu row (hearth `menuItem`): `gap-2.5 rounded-lg px-2 py-1.5
...
/// `bg-white/[0.08]` (hearth's `data-[highlighted]` styling) — two selected-
...
/// Small uppercase section heading inside a floating menu (hearth
...
/// Hairline divider between menu sections (hearth `MenuSeparator`:
...
/// The search/text input frame at the top of a picker popover (hearth
...
/// A bordered trailing menu section (hearth picker action groups /
...
// ---------------------------------------------------------------------------
// Dialog primitives (hearth dialog.tsx / sidebar dialogs.tsx)
// ---------------------------------------------------------------------------
...
/// `transition-colors` (hearth dialogs.tsx). Caller adds id + click; `fade_key`
/// as in [`menu_row`].
...
/// Pulsing skeleton rows shown while a list loads (hearth:
...
pub fn skeleton_rows(
    ...
    let wash = ink(0.04);
    let delta = motion::pulse_delta(&HEARTH_PULSE, view, cx);
    div()
        .flex()
        .flex_col()
        .gap(px(6.0))
