...
use hearth_doc::{MessagePart, MessageRole, SessionCommandPayload, SessionMessageEntry};
use hearth_proto::{
    FileSearchMatch, HarnessId, RunRequest, SandboxLevel, SlashCommand, UserInputAnswer,
    UserInputQuestion,
};
use hearth_rpc::{RpcError, methods};
...
/// Expanded-mode textarea vertical padding: `pt-4 pb-1` (hearth composer.tsx
/// line 578) = 16 + 4.
...
/// 260)` (hearth composer.tsx line 235). The 76px floor applies even when
...
/// children; composer/styles.tsx pickerChip) + `pb-2.5` (10) — hearth
...
/// Staged-attachment strip metrics (hearth attachment-ui.tsx AttachmentStrip:
...
const FILE_MENTION_SCHEME: &str = "hearth-file:";
...
fn mention_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older hearth — update it to search its files".into()
        }
...
fn slash_error_message(err: &RpcError) -> SharedString {
    match err {
        RpcError::UnknownMethod(_) => {
            "The session's device runs an older hearth — update it to list commands".into()
        }
        RpcError::Transport(_) | RpcError::Closed => "The session's device is unreachable".into(),
...
pub struct Composer {
    ...
    /// `Window` — the `HEARTH_ATTACH_PREVIEW` boot knob opens in `new`).
...
impl Composer {
    ...
    pub fn new(state: Entity<AppState>, cx: &mut Context<Self>) -> Self {
        ...
        // a rig) — `HEARTH_ATTACH=/path/a.png[,/path/b.png]`, and
        // `HEARTH_ATTACH_PREVIEW=1` boots with the first one's lightbox open.
        if let Ok(spec) = std::env::var("HEARTH_ATTACH") {
            let staged: Vec<StagedAttachment> = spec
                ...
                .filter_map(|path| {
                    match attachments::stage_file(std::path::Path::new(path.trim())) {
                        Ok(att) => Some(att),
                        Err(err) => {
                            tracing::warn!(%path, error = %err, "HEARTH_ATTACH stage failed");
                            None
                        }
                    }
                })
                .collect();
            if std::env::var("HEARTH_ATTACH_PREVIEW").is_ok_and(|v| v == "1")
    ...
    /// Capture-knob passthrough (`HEARTH_OPEN_DIALOG=model`): open the
    ...
    fn send(&mut self, text: String, steer: bool, cx: &mut Context<Self>) {
        ...
        self.send_task = Some(cx.spawn(async move |this, cx| {
            let result: Result<(), String> = async {
                ...
                let mut run_worktree: Option<hearth_proto::WorktreeSpec> = None;
                ...
                if is_new {
                    match &plan {
                        ...
                        crate::pickers::CheckoutPlan::NewWorktree { base } => {
                            ...
                            // actual hearth/<name> branch post-creation. cwd
                            ...
                            if let Some(repo_path) = &space_path {
                                ...
                                run_worktree = Some(hearth_proto::WorktreeSpec {
    ...
    /// The agent-asked-a-question panel (hearth question-panel.tsx), rendered in
    ...
    fn render_wizard(&mut self, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        let options = question.options.iter().enumerate().map(|(ix, label)| {
            ...
            // (typed answers win — hearth question-panel.tsx `isSel`).
            ...
            div()
                ...
                // hearth question-panel.tsx option rows: `transition-colors`.
                .bg(if picked {
    ...
    fn render_send_button(
        ...
        let theme = Theme::of(cx);
        // Hearth composer-actions.tsx: a size-7 filled circle — up-arrow to
...
impl Render for Composer {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Centered composer column (hearth `mx-auto w-full max-w-3xl`).
        let container = div()
            .w_full()
            ...
            .when_some(failure, |el, message| {
                // hearth composer.tsx `Notice` (matches the transcript
        ...
        let steer_queues = mode == SendButtonMode::Steer
            && self.pickers.read(cx).resolved_steering_mode(cx)
                == Some(hearth_proto::SteeringMode::TurnBoundary);
        ...
        // need the full-width actions row (hearth composer-actions.tsx
        // `mustExpand = isNew || …`).
        ...
        let attach = div()
            ...
            .rounded_full()
            .cursor_pointer()
            // hearth composer-actions.tsx attach: `transition-colors`.
        ...
        // The pill chrome (hearth composer.tsx): `rounded-[26px] border
...
mod tests {
    ...
    fn file_mentions_serialize_to_strict_local_markdown() {
        ...
        assert_eq!(
            raw,
            "[a file#\\[x\\].rs](hearth-file:src/a%20file%23%5Bx%5D.rs)"
        );
        let links = file_mention_links(&raw);
        ...
        assert_eq!(folder, "[components](hearth-file:src/components/)");
    ...
    fn sent_mention_display_leaves_plain_prompts_untouched() {
        ...
        assert_eq!(
            sent_mention_display("what is a hearth-file: link?"),
            None,
            "scheme substring without a valid mention link"
        );
        assert_eq!(
            sent_mention_display("[a.rs](hearth-file:../a.rs)"),
            None,
    ...
    #[test]
    fn auto_grow_math() {
        // The source heights (hearth composer.tsx line 235 clamp, composer-
        ...
        // Caps at a 260px textarea box (hearth max-h-[260px] / the JS clamp).
        assert_eq!(
