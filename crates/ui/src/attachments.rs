...
//!
//! Ports of hearth's `composer/use-attachments.ts` (staging/upload),
...
/// `(body_end, refs_start)` byte offsets — the tolerant equivalent of hearth's
/// `ATTACHED_IMAGES_RE`.
...
/// the same stuck-offset guard as hearth's `readAttachmentImage`).
