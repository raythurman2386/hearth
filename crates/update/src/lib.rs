...
/// `hearth-<ver>-<os>-<arch>.tar.gz` — the headless/CLI tarball (Linux CI builds).
pub fn headless_artifact(version: &str) -> String {
    let (os, arch) = platform_key();
    format!("hearth-{version}-{os}-{arch}.tar.gz")
}

/// `hearth-<ver>-macos-<arch>-app.tar.gz` — the macOS app update payload.
pub fn mac_app_artifact(version: &str) -> String {
    let (_, arch) = platform_key();
    format!("hearth-{version}-macos-{arch}-app.tar.gz")
}
...
pub enum InstallKind {
    /// `~/.hearth/app/<ver>/hearth` behind the `current` symlink
    /// (curl|sh installer / a previous `hearth update`).
...
fn detect_install_from(exe: &Path, home: Option<&Path>) -> InstallKind {
    if let Some(home) = home {
        ...
        let app_root = home.join(".hearth").join("app");
...
pub async fn stage_headless(
    ...
    let result = async {
        ...
        if !unpacked.join("hearth").is_file() {
            bail!("tarball {file} did not contain a hearth binary");
        }
        match std::fs::rename(&unpacked, &dest) {
            Ok(()) => {}
            // Lost a race with another stager — the staged copy is equivalent.
            Err(_) if dest.join("hearth").exists() => {}
            Err(err) => {
...
/// Download + unpack the app tarball into `{data_dir}/updates/<ver>/Hearth.app`
...
pub async fn stage_mac_app(
    ...
    let staged = dir.join("Hearth.app");
    if staged.join("Contents/MacOS/hearth").exists() {
        return Ok(staged);
    }
    ...
    if !staged.join("Contents/MacOS/hearth").exists() {
        bail!("app tarball {file} did not contain Hearth.app");
    }
    Ok(staged)
}
...
mod tests {
    ...
    fn install_kind_detection() {
        assert_eq!(
            detect_install_from(
                Path::new("/home/u/.hearth/app/0.1.1/hearth"),
                Some(Path::new("/home/u")),
            ),
            InstallKind::Managed {
                app_root: PathBuf::from("/home/u/.hearth/app")
            }
        );
        assert_eq!(
            detect_install_from(
                Path::new("/Applications/Hearth.app/Contents/MacOS/hearth"),
                Some(Path::new("/Users/u")),
            ),
            InstallKind::MacApp {
                bundle: PathBuf::from("/Applications/Hearth.app")
            }
        );
        // A path merely containing `.app` without the bundle layout is not a bundle.
        assert_eq!(
            detect_install_from(Path::new("/tmp/foo.app/hearth"), None),
            InstallKind::Unmanaged
        );
        assert_eq!(
            detect_install_from(
                Path::new("/src/target/release/hearth"),
                Some(Path::new("/home/u"))
            ),
            InstallKind::Unmanaged
        );
    ...
    fn artifact_names_match_packaging() {
        let (os, arch) = platform_key();
        assert!(headless_artifact("0.2.0").starts_with("hearth-0.2.0-"));
        assert_eq!(
            headless_artifact("0.2.0"),
            format!("hearth-0.2.0-{os}-{arch}.tar.gz")
        );
    ...
    fn manifest_parses_with_and_without_files() {
        let full: Manifest = serde_json::from_str(
            r#"{"version":"0.1.1","files":{"hearth-0.1.1-linux-x86_64.tar.gz":{"sha256":"abc"}}}"#,
        )
        .unwrap();
        assert_eq!(full.version, "0.1.1");
        assert_eq!(
            full.files["hearth-0.1.1-linux-x86_64.tar.gz"]
                .sha256
                .as_deref(),
            Some("abc")
        );
    ...
    fn headless_symlink_swap() {
        ...
        for ver in ["0.1.0", "0.1.1"] {
            ...
            std::fs::write(app_root.join(ver).join("hearth"), ver).unwrap();
        }
