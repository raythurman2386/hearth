...
A native SwiftUI viewport onto the hearth mesh. The phone is a **peer
...
```sh
cd apps/ios
xcodebuild -project Hearth.xcodeproj -scheme Hearth \
  -destination 'platform=iOS Simulator,name=iPhone 17 Pro' build
```

Or open `Hearth.xcodeproj` in Xcode and run. Dependencies (SPM, resolved
