//! `hearth daemon …` — install/manage `hearth headless` as a background service:
...
pub fn install(data_dir: &Path) -> anyhow::Result<()> {
    let exe = std::env::current_exe().context("resolving the hearth executable path")?;
    ...
    if cfg!(target_os = "macos") {
        ...
        bail!("hearth daemon is only supported on macOS (launchd) and Linux (systemd)");
    }
    println!(
...
pub fn start() -> anyhow::Result<()> {
    if cfg!(target_os = "macos") {
        let plist = launchd_plist_path()?;
        if !plist.exists() {
            bail!("not installed — run `hearth daemon install` first");
        }
        ...
        if !output.status.success() {
            println!(
                ...
                if launchd_plist_path()?.exists() {
                    " (installed — `hearth daemon start`)"
                } else {
                    " (not installed — `hearth daemon install`)"
                }
            );
            return Ok(());
        }
...
fn render_systemd_unit(exe: &Path, env: &[(String, String)]) -> String {
    let mut unit = String::from(
        "[Unit]\nDescription=Hearth headless engine\nAfter=network-online.target\nStartLimitIntervalSec=60\nStartLimitBurst=5\n\n[Service]\n",
    );
...
/// The ExecStart binary path. An exe under `~/.hearth/app/` came from the
...
fn exec_path_for(exe: &Path, home: Option<&Path>) -> String {
    let installed = home
        .map(|home| home.join(".hearth/app"))
        .is_some_and(|app_root| exe.starts_with(app_root));
    if installed {
        "%h/.hearth/app/current/hearth".to_string()
    } else {
        format!("{}", exe.display())
    }
}
...
mod tests {
    ...
    fn installed_exe_uses_the_current_symlink() {
        ...
        assert_eq!(
            exec_path_for(
                Path::new("/home/u/.hearth/app/0.3.0/hearth"),
                Some(Path::new("/home/u")),
            ),
            "%h/.hearth/app/current/hearth"
        );
        // Source build: literal path.
        assert_eq!(
            exec_path_for(
                Path::new("/src/target/debug/hearth"),
                Some(Path::new("/home/u"))
            ),
            "/src/target/debug/hearth"
        );
    }
    ...
    fn launchd_plist_shape() {
        ...
        assert!(plist.contains("<string>/Users/x/hearth &amp; co/hearth</string>"));
