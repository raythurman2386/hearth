...
#
# Downloads the prebuilt Linux binary from the latest GitHub Release, verifies
# its sha256 against the release manifest, and puts `hearth` on PATH. No build,
# no root, no background service — the engine daemon stays opt-in via
# `hearth daemon install` (see docs/configuration.md).
#
# Or point at a hub that already has releases published:
#   HEARTH_BASE_URL=http://minis.YOUR-TAILNET.ts.net:27655 sh install.sh
#
# Re-running upgrades in place; ~/.hearth state is preserved.
set -eu
...
echo ""
echo "next: run \`hearth\` to open the UI."
echo "optional: \`hearth daemon install\` to run the engine as a background service."
