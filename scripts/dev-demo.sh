...
#
# Everything lives under /tmp/hearth-demo-*; re-runs reuse it. Ctrl-C cleans up.
set -euo pipefail
cd "$(dirname "$0")/.."

DAEMON_DIR=/tmp/hearth-demo-daemon
UI_DIR=/tmp/hearth-demo-ui
IPC=27921
DELAY=""
[[ "${1:-}" == "--slow" ]] && DELAY=350

echo "▸ building (first run takes a few minutes)…"
cargo build -p hearth -q

echo "▸ starting engine daemon on :$IPC"
env HEARTH_DATA_DIR="$DAEMON_DIR" HEARTH_IPC_PORT=$IPC HEARTH_HARNESS=mock \
  ${DELAY:+HEARTH_MOCK_DELAY_MS=$DELAY} RUST_LOG=warn \
  ./target/debug/hearth headless &
DAEMON_PID=$!
...
probe() { cargo run -q -p hearth-rpc --example rpc_probe -- "ws://127.0.0.1:$IPC" "$@"; }
...
if [[ ! -f "$DAEMON_DIR/.demo-seeded" ]]; then
  ...
  declare -A SPACES=()
  for project in hearth soccertcg hearth aether; do
  ...
  seed "Native Hearth Rust Rewrite"    hearth hearth/main                 0  run
  seed "Rebalance Player Stats Caps"  soccertcg    hearth/rebalance-player-stat-caps  2  run
  seed "Craft Premium TCG Experience" soccertcg    hearth/craft-premium-tcg-exp       26 skip
  seed "Initial Context Exploration"  hearth        hearth/initial-context-exploration 14 skip
  seed "Soccer TCG Repo Creation"     aether       aether/main                       48 skip
  touch "$DAEMON_DIR/.demo-seeded"
fi

echo "▸ opening hearth (composer is live — type into it; --slow shows streaming)"
HEARTH_DATA_DIR="$UI_DIR" HEARTH_IPC_PORT=$IPC RUST_LOG=warn ./target/debug/hearth
