...
# hearth-rpc e2e_driver example proving the doc-queued cross-device command path:
#
#   B queues a Run into the chat doc -> nudge -> A (host) executes via the mock
#   harness -> transcript + session status sync A -> edge -> B.
#
# Both engines run as the SAME user (alice@org1) on different devices — hearth's
# one-user-many-devices model; chat/device rooms are claim-on-first-join per user.
#
# Usage: scripts/e2e-smoke.sh
# Env:   HEARTH_E2E_EDGE_PORT (default 27640), HEARTH_E2E_KEEP_LOGS=1 to keep logs.

set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
command -v cargo >/dev/null 2>&1 || PATH="$HOME/.cargo/bin:$PATH"
EDGE_PORT="${HEARTH_E2E_EDGE_PORT:-27640}"
...
# (the target filter applies across every -p), silently skipping the hearth
...
start_engine() { # start_engine <data_dir> <ipc_port> <name> <log>
  HEARTH_DATA_DIR="$1" HEARTH_IPC_PORT="$2" HEARTH_DEVICE_NAME="$3" \
    HEARTH_EDGE_URL="$EDGE_URL" HEARTH_EDGE_TOKEN="$TOKEN" HEARTH_ORG_ID="$ORG" \
