...
#   target/package/hearth-<version>-macos-<arch>.dmg          (user download)
#   target/package/hearth-<version>-macos-<arch>-app.tar.gz   (auto-updater)
# containing Hearth.app (unsigned unless CODESIGN_IDENTITY is set).
#
...
APP="$OUT_DIR/Hearth.app"
DMG="$OUT_DIR/hearth-$VERSION-macos-$ARCH.dmg"
APP_TARBALL="$OUT_DIR/hearth-$VERSION-macos-$ARCH-app.tar.gz"
...
install -m 755 "$ROOT/target/release/hearth" "$APP/Contents/MacOS/hearth"
...
# applied ahead of time; dist/hearth.png stays the full-bleed shared artwork).
ICONSET="$OUT_DIR/hearth.iconset"
...
if $NOTARIZE; then
  ...
  # Gatekeeper offline.
  ZIP="$OUT_DIR/hearth-notarize.zip"
...
tar -czf "$APP_TARBALL" -C "$OUT_DIR" Hearth.app
...
dmgbuild.build_dmg(
    ...
    settings={
        ...
        "icon": os.path.join(app, "Contents/Resources/hearth.icns"),
        ...
        "icon_size": 104,
        "text_size": 12,
        "icon_locations": {"Hearth.app": (165, 195), "Applications": (495, 195)},
    },
)
PY
rm -f "$BG_TIFF"
